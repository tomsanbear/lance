    ...
    pub async fn into_reverse_sorted_data_stream(self) -> Result<SendableRecordBatchStream> {
        ...
        let batches = page_stream
            ...
            .map_ok(move |batch| {
                ...
                let reversed_indices: UInt32Array = (0..num_rows).rev().map(|i| i as u32).collect();

                // Take rows in reversed order
                let reversed_col0 =
                    take::take(batch.column(0).as_ref(), &reversed_indices, None).unwrap();
                let reversed_col1 =
                    take::take(batch.column(1).as_ref(), &reversed_indices, None).unwrap();

                RecordBatch::try_new(new_schema.clone(), vec![reversed_col0, reversed_col1]).unwrap()
            })
            .boxed();
