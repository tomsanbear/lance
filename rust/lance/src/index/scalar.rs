...
pub(super) async fn build_compound_btree_index(
        ...
        .map(|col| {
            dataset
                ...
                .ok_or_else(|| Error::invalid_input(
                    format!("No column with name {}", col),
                ))
                .map(|f| f.into())
        })
    ...
    if fields.is_empty() {
        return Err(Error::invalid_input(
            "Compound index requires at least one column".to_string(),
        ));
    }
    ...
    let params_json = serde_json::to_string(&compound_params).map_err(|e| {
        Error::internal(format!("Failed to serialize compound params: {}", e))
    })?;
    ...
    let mut fields: Vec<arrow_schema::Field> = columns
        ...
        .map(|col| {
            let field = dataset.schema().field(col).ok_or_else(|| {
                Error::invalid_input(format!("No column with name {}", col))
            })?;
